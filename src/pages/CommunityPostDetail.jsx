...
export default function CommunityPostDetail() {
  ...
  useEffect(() => {
   ...
   const handleLike = async () => {
    try {
      ...
      await axios.post(
        `http://13.211.69.139:8080/api/${type_}/posts/${id}/like?userId=${localStorage.getItem('userId')}`,
      );
      fetchData();
    } catch (err) {
  ...
  const handleCommentSubmit = async () => {
    ...
    try {
      ...
      // console.log(id)
      await axios.post(
        `http://13.211.69.139:8080/api/${type_}/posts/${id}/comments`,
        comment,
        {
          headers: {
  ...
  return (
    <>
      ...
      <S.ContentContainer>
          ...
          <S.SmallLine/>
          <S.FlexContainer>
            <img  src={data.liked ? star_red : star_gray} style={{width:'16px',height:'16px',marginBottom:'4px',cursor:'pointer'}}  onClick={handleLike}/>
        ...
        {data?.comments?.map((comment)=>{
          return(
            <React.Fragment key={comment.id}>
              <S.CommentContainer>
                <S.CommentNickName>{comment.nickname ? comment.nickname : '익명'}</S.CommentNickName>
      ...
      <S.InputWrapper>
        <S.InputContainer>
          ...
          <S.Button 
            ...
            onClick={handleCommentSubmit}>댓글 등록</S.Button>
        </S.InputContainer>
      </S.InputWrapper>
    </>
  );
